...
<body>
    ...
    <script>
// Ensure product info is below image (already in correct order)
// Get Quote button logic
function scrollToContactAndFill(product, category) {
    var info = product + (category ? ' (' + category + ')' : '');
    document.getElementById('productInterest').value = info;
    var message = document.getElementById('message');
    if (message) {
        // Only add product info if not already present at the top
        if (!message.value.startsWith('Product: ' + info)) {
            message.value = 'Product: ' + info + '\n' + (message.value.replace(/^Product:.*\n/, ''));
        }
    }
